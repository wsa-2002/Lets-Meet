...
export default () => {
  ...
  const EventTemplate = (temp) => (event) => {
    ...
    if (temp === "month") {
      ...
      style = {
        ...style,
        padding: `${RWDHeight(10)} ${RWDWidth(12)}`,
        whiteSpace: "normal",
        wordWrap: "break-word",
        height: "fit-content",
        minHeight: "100%",
      ...
      if (event.category === "time") {
        return (
          <div style={style}>{`${temp} ${moment(new Date(event.start)).format(
            "HH:mm"
          )}`}</div>
        );
      } else {
  ...
  useEffect(() => {
    ...
    setCalendarOption((prev) => ({
      ...
      template: {
        ...prev.template,
        time: EventTemplate(mode),
        allday: EventTemplate(mode),
      },
    }));
  }, [mode]);
