...
async def get_event(account_id: str, start_date: date, end_date: date):
    sql, params = pyformat2psql(
        sql=fr"  SELECT invite_code, meet.finalized_start_date, meet.finalized_end_date, title, finalized_start_time_slot_id, finalized_end_time_slot_id" 
            fr"  FROM event"
            fr"  INNER JOIN meet"  
    ...
    return [calendar.CalendarEvent(invite_code=invite_code, title=title, start_date=finalized_start_date, 
